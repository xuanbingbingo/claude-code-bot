...
| 命令 | 说明 |
|------|------|
| `/voice` | 查看当前会话的语音开关与音色 |
| `/voice on` | 开启语音回复（默认即开） |
| `/voice off` | 关闭语音回复（文字照常发） |
| `/voice list` | 列出全部可用音色（按引擎分组，不可用的单独标注） |
| `/voice <音色名>` | 换音色并当场发一条试听；名字 / 别名 / 编号都认（`磁性` = `12` = `磁性男声`） |

音色按会话记，与 `BOT_VOICE_NAME` 的关系是：**会话内 `/voice <音色>` > 该 bot 的 `BOT_VOICE_NAME` > hfvoice 默认（清爽男声）**。
音色表直接读 `~/aiProjects/hf-voice/voices.json`（唯一真相源），加音色不用改网关代码。
本地 kokoro 那 8 个（`k1`–`k8`）模型文件缺失时会自动标为不可用并拦住，不会出现「切了却没声音」。
